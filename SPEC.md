...
| draft-03 (hardening) | 2026-04-18 | `0.2.0-alpha.2` | No wire or API change; two receiver-side additions landed in the crate without spec-surface changes: rekey-aware fingerprint verify probing both current and previous keys during grace window; cargo-fuzz target for `observe_consent`; violation-event-sequence test vectors 10/11/12. |
| draft-03 (hardening) | 2026-04-18 | `0.2.0-alpha.3` | No wire or API change; SPEC §12.3.1 tightened: dual fingerprint derivation during rekey grace is now a MUST (closes a timing distinguisher — one HKDF on match vs two on mismatch leaks which epoch signed); §12.3.1 gained a "design-evolution" note on the current-key vs initial-key choice; §12.6 `LegacyBypass` documented bluntly as intentional compatibility mode; Appendix A vectors 07/08 relabeled draft-03 (stale caption). |
