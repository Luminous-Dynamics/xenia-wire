...
proptest! {
    ...
    fn replay_window_sequential_always_accepts(n in 0u64..=200) {
        ...
        for seq in 0..=n {
            prop_assert!(w.accept(0x1234_5678_9ABC_DEF0, 0x10, 0u32, seq));
        }
    }
    ...
    fn replay_window_duplicate_always_rejects(seq in 0u64..1_000_000) {
        let mut w = ReplayWindow::new();
        prop_assert!(w.accept(0xDEAD_BEEF, 0x10, 0u32, seq));
        prop_assert!(!w.accept(0xDEAD_BEEF, 0x10, 0u32, seq));
    }
    ...
    fn replay_window_streams_are_independent(
        ...
        let mut w = ReplayWindow::new();
        prop_assert!(w.accept(sid1, pt1, 0u32, seq));
        // Same seq on the OTHER stream is a first-seen on that stream.
        prop_assert!(w.accept(sid2, pt2, 0u32, seq));
    ...
    fn replay_window_advance_past_window_rejects_old(
        ...
        let mut w = ReplayWindow::new();
        prop_assert!(w.accept(0x42, 0x10, 0u32, old_seq));
        prop_assert!(w.accept(0x42, 0x10, 0u32, old_seq + jump));
    ...
    fn replay_window_out_of_order_within_window_accepted_once(
        ...
        let mut w = ReplayWindow::new();
        prop_assert!(w.accept(0x77, 0x10, 0u32, highest));
        let target = highest - offset;
        prop_assert!(w.accept(0x77, 0x10, 0u32, target));
        prop_assert!(!w.accept(0x77, 0x10, 0u32, target));
    }
}
